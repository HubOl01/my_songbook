                ...
                ? [
                      ...
                      onPressed: () async {
                        createExport(
                          ...
                          context.read<SongsBloc>().state is SongsLoaded
                              ? (context.read<SongsBloc>().state as SongsLoaded)
                                  .songGroups
                              : [],
                          () => setState(() {
                            isSecondButton = false;
                    ...
                    IconButton(
                      onPressed: () async {
                        ...
                        if (state is SongsLoaded && selectedSongs.isNotEmpty) {
                          ...
                          if (songGroupSets.isNotEmpty) {
                            ...
                            for (int i = 1; i < songGroupSets.length; i++) {
                              commonGroupIds =
                                  commonGroupIds.intersection(songGroupSets[i]);
                            }
                          ...
                          return IconButton(
                            onPressed: () {
                              if (context.read<IndexGroupCubit>().state == -1) {
                                ...
                                final List<Song> filteredSongs = songGroupLinks
                                    .map((sg) => state.songs
                                        .firstWhere((s) => s.id == sg.songId))
                                    .toList();
                                bool allSelected = filteredSongs.every((song) =>
                                    selectedSongsId.contains(song.id));
                          ...
                          onPointerMove: (event) {
                            ...
                            if (y < edgePadding) {
                              reorderScrollController.jumpTo(
                                  (offset - scrollSpeed).clamp(0.0, maxExtent));
                            } else if (y >
                                ...
                                children: [
                                  for (int index = 0;
                                    ...
                                    ListTile(
                                      ...
                                      minLeadingWidth: 1,
                                      title:
                                          Text(reorderedSongs[index].name_song),
                                      subtitle: Text(
...
Future<void> createExport(
  ...
  //                 title: Text(tr(LocaleKeys.confirmation_title)),
  //                 content:
  //                     Text(tr(LocaleKeys.confirmation_content_export_song)),
  //                 actions: [
  //                   TextButton(
  //                       onPressed: () {
  //                         setState(() {
